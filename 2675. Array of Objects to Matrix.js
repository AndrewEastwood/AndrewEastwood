...
var jsonToMatrix = function(arr) {
    const keys = new Set();
    const flatItems = arr.map(item => {
      const fItem = flattenJSON(item);
      Object.keys(fItem).forEach(k => keys.add(k));
      return fItem;
    });
    const head = Array.from(keys).sort();
    return [
      head,
      ...(flatItems ?? []).map(fItem => {
        return head.map(headKey => headKey in fItem ? fItem[headKey] : '');
      }),
    ];
};

const flattenJSON = (obj, result = {}, path = []) => {
  if (typeof obj === 'object' && obj !== null) {
    for (let key in obj) {
      if (Array.isArray(obj[key]))
        flattenJSON(obj[key], result, [...path, key])
      else
        flattenJSON(obj[key], result, [...path, key])
    }
  } else {
